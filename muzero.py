...
class MuZero:
    ...
    def __init__(self, game_name, config=None, split_resources_in=1):
        ...
        ray.init(num_gpus=total_gpus, ignore_reinit_error=True, include_dashboard=False)
    ...
    def test(
        self, render=True, opponent=None, muzero_player=None, num_tests=1, num_gpus=0, return_all_results=False
    ):
        """
        ...
        Args:
            ...
            return_all_results (bool): should return all reqards instead of mean value
        """
    ...
    if len(sys.argv) == 2:
        ...
        while True:
            ...
            if choice == 0:
                ...
                choice = input("Number of tests: ")
                while not int(choice):
                    choice = input("Number of tests: ")
                results = muzero.test(render=False, opponent="human", num_tests=int(choice), muzero_player=0,
                                      return_all_results=True)
                for history in results:
                    print(history.observation_history[-1])
                    print(history.action_history)
                    print(history.reward_history)
                total_games = len(results)
                win_games = len([True for history in results if history.reward_history[-1] > 0])
                losing_distances = [abs(history.reward_history[-1]) for history in results
                                    if history.reward_history[-1] < 0]
                print(f"win games: {win_games}/{total_games}")
                print(f"win games precentage is {round(win_games/total_games * 100)}%")
                print(f"AVG distance when losing: {sum(losing_distances)/len(losing_distances)}")
            else:
                break
            print("\nDone")
